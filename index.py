...
def check_couple(account_id, couples_data):
...
def get_session():
    ...
    if GLOBAL_SESSION is None:
        ...
        adapter = requests.adapters.HTTPAdapter(pool_connections=500, pool_maxsize=1000, max_retries=0, pool_block=False)
...
# ========== GENERATION ENGINE ==========
def generate_accounts(region, name_prefix, password_prefix, total, threads=100):
    global EXIT_FLAG, SUCCESS_COUNTER, RARE_COUNTER, COUPLES_COUNTER, COUPLES_DATA
    ...
    def generate_single(thread_id):
        nonlocal SUCCESS_COUNTER, RARE_COUNTER, COUPLES_COUNTER
        while not EXIT_FLAG and SUCCESS_COUNTER < total:
            account = create_account(region, name_prefix, password_prefix, is_ghost=(region=="BR"))
            if account and account.get('account_id') != 'N/A':
                SUCCESS_COUNTER += 1
                accounts.append(account)
                if account.get('jwt_token'):
                    tokens.append({
                        'uid': account['uid'],
                ...
                is_rare, _, reason, score = check_rarity(account['account_id'])
                if is_rare:
                    RARE_COUNTER += 1
                ...
                is_couple, creason, partner = check_couple(account['account_id'], COUPLES_DATA)
    ...
    for i in range(threads):
        t = threading.Thread(target=generate_single, args=(i+1,))
        t.daemon = True
        t.start()
    ...
    return {
        ...
        'region': region,
        'total_generated': SUCCESS_COUNTER,
...
def handler(request):
    try:
        ...
        name_prefix = data.get('name', 'JXE')
        password_prefix = data.get('password', 'JXE2026')
        total = int(data.get('total', 10))
        threads = int(data.get('threads', 100))
        category = data.get('type', 'all')  # all, accounts, rare, couples, tokens
        
        if region not in REGION_LANG and region != 'BR':
            region = 'IND'
        
        result = generate_accounts(region, name_prefix, password_prefix, total, threads)
        
        # If specific category requested, filter
        if category == 'accounts':
            response_data = {'accounts': result['accounts'], 'count': len(result['accounts'])}
        elif category == 'rare':
            response_data = {'rare_accounts': result['rare_accounts'], 'count': result['rare_count']}
        elif category == 'couples':
            response_data = {'couple_pairs': result['couple_pairs'], 'count': result['couples_count']}
        elif category == 'tokens':
            response_data = {'tokens': result['tokens'], 'count': len(result['tokens'])}
        else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        class FakeRequest:
            method = 'POST'
            def get_json(self):
                return {'region': 'IND', 'total': 2, 'threads': 2}
            args = {}
        print(handler(FakeRequest()))
    else:
        result = generate_accounts('IND', 'JXE', 'JXE2026', 2, 2)
